...
// import { useScheduler, useGantt, useSummary, Process, FormState, AlgorithmKey, ALGORITHM_LABELS, COLOR_PALETTE } from "../lib/scheduler";

// type ProcessAction =
//   | { type: "ADD"; pid: string; arrival: number; burst: number }
//   | { type: "RESET" };
...
//   const [form, dispatchForm] = useReducer(formReducer, { pid: "", arrival: "", burst: "" } as FormState);
...
//     const { pid, arrival, burst } = form;
//     if (!pid.trim() || arrival === "" || burst === "") {
//       alert("Please fill in all fields.");
//       return;
//     }
//     dispatchProcesses({ type: "ADD", pid: pid.trim(), arrival: parseInt(arrival), burst: parseInt(burst) });
...
//             <InputField label="Process ID" value={form.pid} onChange={handleFieldChange("pid")} placeholder="P1" className="w-28" />
//             <InputField label="Arrival Time" type="number" value={form.arrival} onChange={handleFieldChange("arrival")} placeholder="0" className="w-28" />
//             <InputField label="Burst Time" type="number" value={form.burst} onChange={handleFieldChange("burst")} placeholder="5" className="w-28" />
//             <button onClick={handleAddProcess} className="self-end bg-cyan-500 hover:bg-cyan-400 active:scale-95 text-slate-900 font-bold px-5 py-2 rounded-lg text-sm transition-all">
//               + Add
//             </button>
//             <button onClick={handleReset} className="self-end bg-slate-700 hover:bg-slate-600 active:scale-95 text-white font-bold px-5 py-2 rounded-lg text-sm transition-all">
//               Reset
//             </button>
//           </div>
//         </Panel>
...
//               <label className="text-xs text-slate-400 font-medium">Select Algorithm</label>
//               <select
//                 value={algorithm}
...
//                 type="number"
//                 value={quantum}
//                 onChange={(e: React.ChangeEvent<HTMLInputElement>) => setQuantum(parseInt(e.target.value) || 2)}
//                 placeholder="2"
//                 className="w-28"
//               />
//             )}

//             <button onClick={handleCalculate} className="self-end bg-blue-600 hover:bg-blue-500 active:scale-95 text-white font-bold px-6 py-2 rounded-lg text-sm transition-all">
//               ▶ Calculate
//             </button>
//             <button
//               onClick={handleToggleAnim}
//               className={`self-end font-bold px-5 py-2 rounded-lg text-sm transition-all active:scale-95 ${animEnabled ? "bg-emerald-600 hover:bg-emerald-500" : "bg-red-700 hover:bg-red-600"} text-white`}
//             >
...
//           <ProcessTable processes={processes} stats={calculated ? stats : null} />
//         </Panel>
...
//             <div>
//               <p className="text-xs uppercase tracking-widest text-blue-400 mb-1">Algorithm</p>
//               <p className="font-bold text-white">{ALGORITHM_LABELS[algorithm]}</p>
//             </div>
//             <div>
//               <p className="text-xs uppercase tracking-widest text-blue-400 mb-1">Avg Turnaround Time</p>
//               <p className="font-bold text-cyan-300 text-2xl">{summary.avgTAT}</p>
//             </div>
//             <div>
//               <p className="text-xs uppercase tracking-widest text-blue-400 mb-1">Avg Waiting Time</p>
...
//             <GanttChart segments={gantt.segments} animEnabled={animEnabled} ganttKey={ganttKey} />
//           </Panel>
//         )}
